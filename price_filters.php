...
} elseif(isset($_POST['product_price']) && isset($_POST['tagId'])) {
    $where_condition = "WHERE id IN (SELECT product_id FROM grocery_product_bind_tags WHERE tag_id = '".$_POST['tagId']."') AND lkp_status_id = 0 AND id IN (SELECT product_id FROM grocery_product_bind_weight_prices WHERE lkp_status_id = 0 AND lkp_city_id = '$lkp_city_id' AND (selling_price BETWEEN '$sendMinPrice' AND '$sendMaxPrice'))  ORDER BY id DESC";
} elseif($_POST['product_price'] == '' && isset($_POST['tagId'])) {
    $where_condition = "WHERE id IN (SELECT product_id FROM grocery_product_bind_tags WHERE tag_id = '".$_POST['tagId']."') AND lkp_status_id = 0 AND id IN (SELECT product_id FROM grocery_product_bind_weight_prices WHERE lkp_status_id = 0 AND lkp_city_id = '$lkp_city_id')  ORDER BY id DESC";
} else {
